...
const Vector3 Vector3::ZERO_VECTOR = Vector3(0.f, 0.f, 0.f);
...
Vector3 Vector3::Move(Vector3 currentPos, Vector3 newPos, float speedwithDT, bool away)
{
	currentPos.y = newPos.y;
	float Distance = (currentPos - newPos).LengthSquared();
	if (away == false)
	{
		if (Distance > speedwithDT*speedwithDT)
		{
			return currentPos + (currentPos - newPos).Normalized() * speedwithDT;
		}
		else
		{
			return newPos;
	...
	{
		return currentPos + (newPos - currentPos).Normalized() * speedwithDT;
	}
}