...
const float m_fTPVCameraOffset = 30.f;	// Offset distance for the camera from the target
...
void Camera3::Update(double dt)
{
 ...
 void Camera3::UpdatePosition(Vector3 newPos, Vector3 newDir)
 {
	 position = newPos - newDir.Normalized() * m_fTPVCameraOffset; // Setting camera position to the opposite direction of avatar with value of m_fTPVCameraOffset
	 position.y += 10;
	 target = newPos + newDir;
	 target.y += 10;
 }
